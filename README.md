...
- 项目预算：创建预算、查询预算（返回总额、已用、预占和可用额）、提交审批、审批通过、审批驳回。
- 成本项：录入成本、自动计算差异金额、核对差异、标记异常。
- 变更单：关联一份已审批预算后发起变更，草稿不占额度；提交时按变更金额预占预算可用额，余额不足自动退回草稿并写明原因；审批通过后预占并入预算总额，驳回或作废释放预占。并发提交以数据库行锁串行化，重复提交、重复审批均不会重复增减。
- 成本分析报告：按项目生成月度/季度/年度/自定义报告，汇总人工、材料、设备和其他成本；预算口径只含已审批预算（含已并入的变更金额，即调整后预算）。
